...
def main():
    # 명령줄 인자 파싱
    parser = argparse.ArgumentParser(description='활동 인식 모델 학습')
    parser.add_argument('--device', type=str, choices=['cpu', 'cuda', 'auto'], default='cpu',
                      help='사용할 장치 선택 (cpu, cuda, auto). 기본값은 cpu')
    ...
    if args.device == 'auto':
        ...
        if args.device == 'cuda' and not torch.cuda.is_available():
            print("경고: CUDA를 선택했으나 사용할 수 없습니다. CPU로 대체합니다.")
    ...
    config = {
        ...
        "model": {
            ...
            "transition_rules": {
                ...
                "transition_matrix": [
                    [1, 1, 1, 1, 1],  # standing -> *
                    [1, 1, 0, 1, 1],  # sitting -> * (walking 불가)
                    [0, 1, 1, 1, 1],  # walking -> * (standing 불가)
                    [1, 1, 1, 1, 1],  # no_activity -> *
