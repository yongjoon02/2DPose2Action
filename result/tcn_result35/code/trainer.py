...
def calculate_overlap_f1(true_segments, pred_segments, thresholds=[0.25, 0.5]):
    """
    활동 세그먼트 간의 겹침(overlap)을 기반으로 F1 Score를 계산합니다.
    
    Args:
        true_segments: 실제 활동 세그먼트 리스트 [start, end, class_id]
        pred_segments: 예측 활동 세그먼트 리스트 [start, end, class_id]
        thresholds: 겹침 임계값 리스트 (예: [0.25, 0.5])
...
def extract_activity_segments(labels):
    """
    연속된 프레임 레이블을 활동 세그먼트 리스트로 변환합니다.
    
    Args:
        labels: 프레임별 레이블 배열
        
    Returns:
        세그먼트 리스트 [start, end, class_id]
    """
    if len(labels) == 0:
        return []
...
def test_ensemble(models, dataloader, device, output_dir, combined_results_dir, config, final_results):
    ...
    all_labels_list = []  # 각 샘플의 전체 레이블 시퀀스 저장
    all_preds_list = []   # 각 샘플의 전체 예측 시퀀스 저장
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="테스트 중"):
            ...
            for i in range(coords.shape[0]):
                ...
                # 레이블이 있는 경우 전체 시퀀스 저장
                if labels is not None:
                    true_labels = labels[i].cpu().numpy()
                    all_labels_list.append(true_labels)
                    all_preds_list.append(pred_labels)
    ...
    # 기존의 프레임별 평가 (혼동 행렬, 분류 보고서 등)
    # ... 기존 코드 ...
    
    # ===== F1 Overlap Score 계산 추가 =====
    if all_labels_list and all_preds_list:
        print("\nF1 Overlap Score 계산 중...")
        
        # 모든 샘플의 활동 세그먼트 추출
        all_true_segments = []
        all_pred_segments = []
        
        for true_labels, pred_labels in zip(all_labels_list, all_preds_list):
        ...
        # 여러 임계값에 대해 F1 Overlap Score 계산
        thresholds = [0.25, 0.5]
        overlap_results = calculate_overlap_f1(all_true_segments, all_pred_segments, thresholds)
        ...
        try:
            ...
            thresholds_list = list(overlap_results.keys())
            f1_scores = [overlap_results[t]['f1'] for t in thresholds_list]
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(combined_results_dir, "overlap_f1_chart.png"), dpi=100)
            plt.close()
            
            print(f"F1 Overlap Score 차트가 저장되었습니다.")
        except Exception as e:
            print(f"차트 저장 오류: {str(e)}")
        
        # 혼동 행렬 PNG 저장 (확실히 처리)
        try:
            print("혼동 행렬 시각화 저장 중...")
            ...
            for fmt in ['.png', '.jpg', '.pdf']:
                save_path = os.path.join(combined_results_dir, f"confusion_matrix{fmt}")
