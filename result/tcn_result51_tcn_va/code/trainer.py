...
def test_ensemble(models, dataloader, device, output_dir, combined_results_dir, config, final_results):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="테스트 중"):
            ...
            # 추가 인자 제거하여 바닐라 TCN 모델 호출
            logits = models[0](coords)
...
def test(model, dataloader, device, save_dir=None, dataset_path=None, return_predictions=False):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="테스트 중"):
            ...
            # 튜플로 반환되는 경우 처리 (바닐라 TCN에서는 항상 텐서만 반환)
            logits = outputs
            _, predictions = torch.max(logits, dim=-1)
...
def train_model(dataset, config):
    ...
    # 손실 함수와 옵티마이저 설정 - 단순한 크로스 엔트로피 손실과 아담 옵티마이저
...
def calculate_and_save_f1_overlap(test_loader, models, device, save_dir):
        ...
        for batch in test_loader:
            ...
            for i in range(coords.shape[0]):
                if labels is not None:
                    ...
                    true_segs = extract_activity_segments(true_labels)
                    pred_segs = extract_activity_segments(pred_labels)
                    
                    # Overlap F1 Score 계산
                    overlap_results = calculate_overlap_f1(true_segs, pred_segs)
                    
                    # 결과 저장
                    for threshold, metrics in overlap_results.items():
                        save_path = os.path.join(save_dir, f"overlap_f1_results_{threshold*100:.0f}.csv")
    ...
    print(f"F1 Overlap Score 결과가 {save_dir}에 저장되었습니다.")