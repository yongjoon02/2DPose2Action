...
def test_ensemble_with_overlap_f1(models, dataloader, device, output_dir, combined_results_dir, config, final_results):
    ...
    if all_labels:
        ...
        print(f"\nF1 Overlap Score 결과:")
        for threshold, metrics in overlap_results.items():
            print(f"  임계값 {threshold*100:.0f}%: F1={metrics['f1']:.4f}")
...
def calculate_overlap_f1_direct(true_segments, pred_segments, save_dir, thresholds=[0.25, 0.5]):
    """F1 Overlap Score를 직접 계산하고 결과를 명확하게 저장"""
    results = {}
    ...
    # 결과를 저장할 디렉토리가 존재하는지 확인
    os.makedirs(save_dir, exist_ok=True)
    
    # 직접 결과를 텍스트 파일에 쓰기
    txt_path = os.path.join(save_dir, f"overlap_f1_results_{timestamp}.txt")
    csv_path = os.path.join(save_dir, f"overlap_f1_results_{timestamp}.csv")
    
    try:
        with open(txt_path, "w", encoding="utf-8") as f:
            f.write(f"F1 Overlap Score 결과 (생성 시간: {time.ctime()})\n\n")
            f.write(f"전체 세그먼트 수: 실제={len(true_segments)}, 예측={len(pred_segments)}\n\n")
            ...
            for threshold in thresholds:
                ...
                for t_idx, (t_start, t_end, t_class) in enumerate(true_segments):
                    ...
                    f.write(f"세그먼트 #{t_idx}: 클래스={t_class}, 범위=[{t_start},{t_end}]\n")
                    if best_overlap >= threshold:
                        tp += 1
                        matched_preds.add(best_pred_idx)
                        f.write(f"  결과: TP (겹침률={best_overlap:.4f})\n")
                    else:
                        f.write(f"  결과: FN (최대 겹침률={best_overlap:.4f})\n")
        ...
        print(f"F1 Overlap Score 계산 결과가 저장되었습니다: {txt_path}")
        ...
        print(f"F1 Overlap Score CSV 결과가 저장되었습니다: {csv_path}")
        
    except Exception as e:
        print(f"F1 Overlap Score 결과 저장 중 오류: {str(e)}")
        traceback.print_exc()
