...
def test_ensemble(models, dataloader, device, output_dir, combined_results_dir, config, final_results):
    ...
    if all_labels:
        ...
        for cls_idx, cls_name in enumerate(class_names):
            ap = calculate_ap(all_labels, all_scores, cls_idx)
            ap_scores[cls_name] = ap * 100  # 퍼센트로 변환
        
        mAP = np.mean(list(ap_scores.values()))
        print(f"mAP: {mAP:.2f}%")
        
        # mAP 결과 저장
        with open(os.path.join(combined_results_dir, "map_results.txt"), "w", encoding="utf-8") as f:
            f.write(f"Mean Average Precision (mAP): {mAP:.2f}%\n\n")
            f.write("클래스별 Average Precision:\n")
            for cls_name, ap in ap_scores.items():
                f.write(f"{cls_name}: {ap:.2f}%\n")
        
        # mAP 시각화
        plt.figure(figsize=(10, 6))
        plt.bar(ap_scores.keys(), ap_scores.values())
        plt.title('Average Precision by Class')
        plt.xlabel('Class')
        plt.ylabel('AP (%)')
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig(os.path.join(combined_results_dir, "ap_by_class.png"))
        plt.close()
        
        # 기존 결과에 mAP 추가
        final_results["mAP"] = float(mAP)
        final_results["class_AP"] = {k: float(v) for k, v in ap_scores.items()}
        
        # 결과 저장
        with open(os.path.join(combined_results_dir, "test_results.txt"), "a", encoding="utf-8") as f:
            f.write(f"\nmAP: {mAP:.2f}%\n")
            f.write("\n클래스별 AP:\n")
            for cls_name, ap in ap_scores.items():
                f.write(f"{cls_name}: {ap:.2f}%\n")
        ...
        # 배치 단위가 아닌 전체 시퀀스에 대해 한 번에 계산
        edit_score, segment_info = calculate_segmental_edit_score(all_labels, all_preds)
        print(f"Segmental Edit Score: {edit_score:.2f}%")
        
        # 세그먼트 평가 결과 저장
        with open(os.path.join(combined_results_dir, "segmental_results.txt"), "w", encoding="utf-8") as f:
            f.write(f"Segmental Edit Score: {edit_score:.2f}%\n\n")
            f.write("세그먼트 통계:\n")
            f.write(f"실제 세그먼트 수: {segment_info['num_true_segments']}\n")
            f.write(f"예측 세그먼트 수: {segment_info['num_pred_segments']}\n")
            f.write(f"Edit Distance: {segment_info['edit_distance']}\n")
        
        # 세그먼트 길이 분포 시각화
        plt.figure(figsize=(12, 6))
        
        true_lengths = [end-start+1 for start, end, _ in segment_info["true_segments"]]
        pred_lengths = [end-start+1 for start, end, _ in segment_info["pred_segments"]]
        
        plt.subplot(1, 2, 1)
        plt.hist(true_lengths, bins=30, alpha=0.7, label='True', density=True)
        plt.hist(pred_lengths, bins=30, alpha=0.7, label='Predicted', density=True)
        ...
        plt.subplot(1, 2, 2)
        plt.boxplot([true_lengths, pred_lengths], labels=['True', 'Predicted'])
        plt.ylabel('Segment Length')
        plt.title('Segment Length Box Plot')
        
        plt.tight_layout()
        plt.savefig(os.path.join(combined_results_dir, "segment_analysis.png"))
        plt.close()
        
        # 결과에 추가
        final_results["segmental_edit_score"] = float(edit_score)
        final_results["segment_statistics"] = {
            "num_true_segments": segment_info["num_true_segments"],
            "num_pred_segments": segment_info["num_pred_segments"],
            "edit_distance": segment_info["edit_distance"]
        }
