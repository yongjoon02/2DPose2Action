...
def validate(model, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="검증 중"):
            ...
            # Frame-wise Accuracy 계산을 위해 예측값과 라벨 저장
    ...
    # Frame-wise Accuracy 계산
    all_preds = torch.cat(all_preds)
    all_labels = torch.cat(all_labels)
    frame_accuracy, class_accuracies = TCN.calculate_frame_accuracy(all_preds, all_labels)
    
    # 프레임 정확도를 백분율로 변환
    frame_accuracy = frame_accuracy * 100
    
    return epoch_loss / len(dataloader), accuracy, frame_accuracy, class_accuracies
...
def train_with_early_stopping(model, train_loader, valid_loader, criterion, optimizer, scheduler, device, 
    ...
    best_frame_model_path = os.path.join(fold_dir, "best_frame_model.pth")
    counter = 0
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        valid_loss, valid_acc, frame_acc, class_accuracies = validate(model, valid_loader, criterion, device)
        
        train_loss_history.append(train_loss)
        valid_loss_history.append(valid_loss)
        valid_acc_history.append(valid_acc)
        frame_acc_history.append(frame_acc)
        
        if scheduler is not None:
            scheduler.step()
        
        elapsed = time.time() - start_time
        print(f"Epoch [{epoch}/{num_epochs}], Train Loss: {train_loss:.4f}, Valid Loss: {valid_loss:.4f}, Valid Acc: {valid_acc:.2f}%, Frame Acc: {frame_acc:.2f}%, 시간: {elapsed:.2f}초")
        ...
        # Frame-wise Accuracy 기준 모델 저장
        if frame_acc > best_frame_acc:
            best_frame_acc = frame_acc
            torch.save(model.state_dict(), best_frame_model_path)
        
        if counter >= patience:
            print(f"Early stopping at epoch {epoch}, 최고 검증 정확도: {best_valid_acc:.2f}%, 최고 프레임 정확도: {best_frame_acc:.2f}%")
            break
    ...
    return model, best_valid_acc, best_frame_acc
...
def train_model(dataset, config):
    ...
    for fold, (train_fold_idx, val_fold_idx) in enumerate(kfold.split(np.zeros(len(train_val_idx)), selected_labels), 1):
        ...
        # 모델 학습
        model, best_acc, best_frame_acc = train_with_early_stopping(
            model=model,
        ...
        # 최종 검증 손실 계산
        final_valid_loss, final_valid_acc, final_frame_acc, final_class_accs = validate(model, valid_loader, criterion, device)
        ...
        fold_results.append({
            ...
            "accuracy": best_acc,
            "frame_accuracy": best_frame_acc
        })
        
        print(f"Fold {fold} 완료 - 최고 검증 정확도: {best_acc:.2f}%, 프레임 정확도: {best_frame_acc:.2f}%, 손실: {final_valid_loss:.6f}")
    ...
    # 모든 폴드의 프레임 정확도 수집
    fold_frame_accuracies = [result.get("frame_accuracy", 0) for result in fold_results]
    mean_frame_acc = np.mean(fold_frame_accuracies)
    std_frame_acc = np.std(fold_frame_accuracies)
    
    print(f"\n교차 검증 결과:")
    print(f"평균 정확도: {mean_acc:.2f}% ± {std_acc:.2f}%")
    print(f"평균 프레임 정확도: {mean_frame_acc:.2f}% ± {std_frame_acc:.2f}%")
    ...
    final_results = {
        ...
        "mean_frame_accuracy": float(mean_frame_acc),
        "std_frame_accuracy": float(std_frame_acc),
    ...
    with open(os.path.join(combined_results_dir, "results_summary.txt"), "w", encoding="utf-8") as f:
        ...
        f.write(f"평균 프레임 정확도: {mean_frame_acc:.2f}% ± {std_frame_acc:.2f}%\n")
        f.write(f"평균 검증 손실: {mean_loss:.6f} ± {std_loss:.6f}\n\n")
        f.write("각 폴드 결과:\n")
        for i, result in enumerate(fold_results, 1):
            f.write(f"Fold {i}: 정확도 = {result['accuracy']:.2f}%, 프레임 정확도 = {result.get('frame_accuracy', 0):.2f}%, 손실 = {result['loss']:.6f}\n")
    ...
    def test_ensemble(models, dataloader, device, output_dir, combined_results_dir, config, final_results):
        ...
        if all_labels:
            ...
            # Frame-wise Accuracy 계산
            frame_accuracy, class_accuracies = TCN.calculate_frame_accuracy(all_preds, all_labels)
            frame_acc = frame_accuracy * 100
            
            print(f"테스트 정확도: {test_acc:.2f}%")
            print(f"테스트 프레임 정확도: {frame_acc:.2f}%")
            print("클래스별 프레임 정확도:")
            ...
            with open(os.path.join(combined_results_dir, "results_summary.txt"), "a", encoding="utf-8") as f:
                f.write(f"\n테스트 정확도: {test_acc:.2f}%\n")
                f.write(f"테스트 프레임 정확도: {frame_acc:.2f}%\n")
                f.write("클래스별 프레임 정확도:\n")
                for class_name, acc in class_accuracies.items():
                    f.write(f"  - {class_name}: {acc*100:.2f}%\n")
            
            # 테스트 결과를 final_results에 추가
            final_results["test_accuracy"] = float(test_acc)
            final_results["test_frame_accuracy"] = float(frame_acc)
            final_results["test_class_frame_accuracies"] = {k: float(v*100) for k, v in class_accuracies.items()}
            ...
            # Frame-wise Accuracy 결과 저장
            with open(os.path.join(combined_results_dir, "frame_accuracy.txt"), 'w', encoding='utf-8') as f:
                f.write(f"테스트 프레임 정확도: {frame_acc:.4f}%\n\n")
                f.write("클래스별 프레임 정확도:\n")
                for class_name, acc in class_accuracies.items():
                    f.write(f"{class_name}: {acc*100:.4f}%\n")
            
            # Frame-wise 클래스별 정확도 CSV 저장
            frame_class_acc_df = pd.DataFrame.from_dict({k: v*100 for k, v in class_accuracies.items()}, 
                                                       orient='index', columns=['Frame Accuracy'])
            frame_class_acc_df.index.name = 'Class'
            frame_class_acc_df.to_csv(os.path.join(combined_results_dir, "frame_class_accuracy.csv"))
...
def save_learning_curves(train_losses, valid_losses, valid_accs, frame_accs, save_path):
    """학습 및 검증 손실과 정확도 내역을 CSV 파일로 저장합니다."""
    with open(save_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['epoch', 'train_loss', 'valid_loss', 'valid_accuracy', 'frame_accuracy'])
        for epoch, (train_loss, valid_loss, valid_acc, frame_acc) in enumerate(zip(train_losses, valid_losses, valid_accs, frame_accs)):
            writer.writerow([epoch + 1, train_loss, valid_loss, valid_acc, frame_acc]) 