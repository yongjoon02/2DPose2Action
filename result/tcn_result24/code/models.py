...
class TCN(nn.Module):
    def __init__(self, input_size, output_size, num_channels, kernel_size, dropout, use_se=False):
        ...
        self.no_activity_threshold = 0.985 # no_activity 클래스(인덱스 3)에 대한 엄격한 threshold
        ...
        self.transition_matrix[0, 3] = 1.0  # standing -> no_activity
        self.transition_matrix[1, 3] = 1.0  # sitting -> no_activity  
        self.transition_matrix[2, 3] = 1.0  # walking -> no_activity
    ...
    @staticmethod
    def calculate_frame_accuracy(predictions, targets):
        """
        프레임 단위 정확도를 계산합니다.
        
        Args:
            predictions (torch.Tensor): 모델의 프레임별 예측값 [batch_size, seq_len] 또는 [total_frames]
            targets (torch.Tensor): 정답 레이블 [batch_size, seq_len] 또는 [total_frames]
            
        Returns:
            float: 프레임 단위 정확도 (0.0 ~ 1.0)
            dict: 클래스별 정확도
        """
        if predictions.shape != targets.shape:
            raise ValueError(f"예측값과 타겟의 형태가 일치하지 않습니다: {predictions.shape} vs {targets.shape}")
        ...
        for cls_idx, cls_name in enumerate(class_names):
            ...
            if cls_mask.sum() > 0:  # 해당 클래스가 존재하는 경우에만
                cls_correct = ((predictions == cls_idx) & cls_mask).float().sum()
                cls_total = cls_mask.float().sum()
                cls_accuracy = (cls_correct / cls_total).item() if cls_total > 0 else 0.0
                class_accuracy[cls_name] = cls_accuracy
        
        return frame_accuracy, class_accuracy
