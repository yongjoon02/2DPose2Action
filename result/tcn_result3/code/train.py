...
def train_epoch(model, dataloader, optimizer, criterion, device):
    ...
    for batch in tqdm(dataloader, desc="훈련 중"):
        ...
        # apply_transition_rules=False로 설정 (CRF 학습 시에는 전이 규칙을 학습에 활용)
        outputs = model(coords, conservative_no_activity=False, apply_transition_rules=True)
        ...
        # CRF 손실 함수 사용 (모델의 loss 메서드 호출)
        B, T, _ = logits.shape
        labels_reshaped = labels.reshape(B, T)
        
        # 마스크 생성 (모든 시퀀스 포인트가 유효하다고 가정)
        mask = torch.ones(B, T, dtype=torch.uint8, device=device)
        
        # CRF 손실 계산
        loss = model.loss(logits, labels_reshaped, mask)
        loss.backward()
...
def validate(model, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="검증 중"):
            ...
            # 모델이 튜플을 반환한 경우 logits와 predictions 추출
            if isinstance(outputs, tuple):
