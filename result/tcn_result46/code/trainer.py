...
def test_ensemble(models, dataloader, device, output_dir, combined_results_dir, config, final_results):
    ...
    if all_labels:
        ...
        for cls_idx, cls_name in enumerate(class_names):
            ...
            if cls_mask.sum() > 0:
                cls_acc[cls_name] = 100 * np.mean(all_preds[cls_mask] == cls_idx)
        ...
        overlap_results = calculate_overlap_f1(
            extract_segments(all_labels),
            extract_segments(all_preds),
            [0.25, 0.5]
        )
        ...
        # 7. 학습 곡선 저장 (이미 존재하는 경우)
        if os.path.exists(os.path.join(combined_results_dir, "learning_curves.csv")):
            shutil.copy(
                os.path.join(combined_results_dir, "learning_curves.csv"),
                os.path.join(combined_results_dir, "learning_curves.csv")
            )
        ...
        final_results.update({
            ...
            "mAP": float(mAP * 100),
            "class_AP": ap_by_class,
            "overlap_f1_scores": {str(int(k*100)): v for k, v in overlap_results.items()},
