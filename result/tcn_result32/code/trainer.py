...
def generate_evaluation_results(model, dataloader, device, fold_dir, class_names):
    ...
    # 혼동 행렬 시각화
    matplotlib.use('Agg')  # GUI가 필요 없는 백엔드로 설정
    
    # 혼동 행렬 저장 함수 - 분리하여 에러 처리를 명확히
    def save_confusion_matrix_plot(cm, class_names, save_path):
        """혼동 행렬을 PNG 파일로 저장합니다."""
        ...
        try:
            ...
            try:
                ...
                print(f"대체 위치에 저장됨: {alt_path}")
            except Exception as e2:
                print(f"대체 저장도 실패: {str(e2)}")
        finally:
            plt.close()
...
def test_ensemble_with_overlap_f1(models, dataloader, device, output_dir, combined_results_dir, config, final_results):
    ...
    if all_labels:
        ...
        # Overlap F1 Score 계산
        overlap_results = calculate_overlap_f1(all_true_segments, all_pred_segments, 
