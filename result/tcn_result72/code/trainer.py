...
from .datasets import class_mapping, SkeletonDataset
from .utils import create_result_dir, save_source_code, calculate_and_save_f1_overlap
...
def process_predictions(pred_sequence):
    ...
    # 클래스 인덱스와 이름 매핑
    class_indices = {0: "standing", 1: "sitting", 2: "walking", 3: "no_activity", 4: "no_presence"}
    ...
    for i in range(1, len(pred_sequence)):
        if pred_sequence[i] != current_class:
            result.append({
                "frameRange": [int(start_frame), int(i)],
                "activity": class_indices[current_class]
            })
            current_class = pred_sequence[i]
            start_frame = i
    
    # 마지막 활동 추가
    result.append({
        "frameRange": [int(start_frame), int(len(pred_sequence))],
        "activity": class_indices[current_class]
    })
        ...
        for batch in tqdm(dataloader, desc="테스트 중"):
            ...
            for i in range(coords.shape[0]):
                ...
                if labels is not None:
                    ...
                    all_scores.append(scores[i].cpu().reshape(-1, scores.shape[-1]))
    
    if all_labels:
        all_labels = torch.cat(all_labels, dim=0).cpu().numpy()
        all_preds = torch.cat(all_preds, dim=0).cpu().numpy()
        all_scores = np.vstack([s.numpy() for s in all_scores])
        ...
        # 2. 클래스별 정확도 계산 및 저장
        # 올바른 클래스 이름 사용: standing, sitting, walking, no_activity, no_presence
        class_names = ["standing", "sitting", "walking", "no_activity", "no_presence"]
        class_acc = {}
        ...
        # 5. AP와 mAP 계산 및 저장
        ap_scores, mAP = calculate_ap_and_map(all_labels, all_scores)
        ap_by_class = {class_names[idx]: score * 100 for idx, score in ap_scores.items()}
        
        # 6. Overlap F1 Score 계산 및 저장
        overlap_results = calculate_overlap_f1(
            extract_activity_segments(all_labels),
            extract_activity_segments(all_preds),
            [0.25, 0.5]
        )
        ...
        # 7. 학습 곡선 저장
        if os.path.exists(os.path.join(combined_results_dir, "learning_curves.csv")):
            # 같은 파일을 복사하려고 시도하지 않음
            pass
        
        # 8. 세그먼트 평가
        edit_score, segment_info = calculate_segmental_edit_score(all_labels, all_preds)
        ...
        final_results.update({
            ...
            "mAP": float(mAP * 100),
            "class_AP": {k: float(v) for k, v in ap_by_class.items()},
            "overlap_f1_scores": {str(int(k*100)): {k2: float(v2) for k2, v2 in v.items()} for k, v in overlap_results.items()},
...
def test(model, dataloader, device, save_dir=None, dataset_path=None, return_predictions=False):
    ...
    all_preds = []
    all_labels = []
    all_scores = []  # 확률 점수를 저장하기 위한 리스트 추가
        ...
        for batch in tqdm(dataloader, desc="테스트 중"):
            ...
            for i, filename in enumerate(filenames):
                ...
                if labels is not None:
                    ...
                    all_scores.append(scores[i].cpu().numpy())  # 확률 점수 저장
    ...
    if all_labels and all_preds:
        ...
        all_scores_array = np.vstack(all_scores)
        ap_scores, mAP = calculate_ap_and_map(all_labels_array, all_scores_array)
        metrics["mAP"] = float(mAP * 100)
        
        # Segmental Edit Score 계산
        edit_score, segments_info = calculate_segmental_edit_score(all_labels_array, np.array(all_preds))
        metrics["segment_edit_score"] = float(edit_score)
        metrics["segment_statistics"] = {
...
def train_model(dataset, config):
    ...
    print(f"- 학습용: {len(train_idx)}개 (80%)")
    print(f"- 검증용: {len(val_idx)}개 (20%)")
    ...
    # 테스트 데이터셋 로드
    test_csv_dir = "data/test_data/test_csv"
    test_json_dir = "data/test_data/test_json"
    
    if os.path.exists(test_csv_dir) and os.path.exists(test_json_dir):
        ...
        print(f"- 테스트용: {len(test_dataset)}개 (별도 테스트셋)")
    else:
        print("경고: 테스트 데이터 디렉토리를 찾을 수 없습니다.")
        test_loader = None
...
def save_confusion_matrix_png(cm, class_names, save_path):
    ...
    def _save_func(path):
        ...
        plt.title('Confusion Matrix', fontsize=16, pad=20)  # 제목 크기
        plt.ylabel('True Class', fontsize=14, labelpad=10)  # y축 라벨
        plt.xlabel('Predicted Class', fontsize=14, labelpad=10)  # x축 라벨
        ...
        plt.tight_layout()
        plt.savefig(path, dpi=150, bbox_inches='tight')  # DPI 증가
        plt.close()
