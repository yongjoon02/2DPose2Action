...
def test_ensemble(models, dataloader, device, output_dir, combined_results_dir, config, final_results):
    ...
    if all_labels:
        ...
        # 데이터셋 크기 정보 추가
        dataset_size = config["dataset_info"]["total_size"]
        train_size = config["dataset_info"]["train_size"]
        val_size = config["dataset_info"]["val_size"]
        
        with open(os.path.join(combined_results_dir, "test_results.txt"), "w", encoding="utf-8") as f:
            f.write(f"데이터셋 크기 정보:\n")
            f.write(f"- 전체 데이터셋 크기: {dataset_size}개\n")
            f.write(f"- 학습 데이터셋 크기: {train_size}개 (80%)\n")
            f.write(f"- 검증 데이터셋 크기: {val_size}개 (20%)\n")
            f.write(f"- 테스트 데이터셋 크기: {test_size}개\n\n")
            f.write(f"테스트 정확도: {test_acc:.2f}%\n")
        
        # 2. 클래스별 정확도 계산 및 저장
        class_names = list(config["classes"])  # 클래스 이름 리스트로 변환
        class_acc = {}
...
def train_model(dataset, config):
    ...
    val_size = int(dataset_size * 0.2)    # 20% 검증용
    
    # 하이퍼파라미터 저장
    config_with_size = config.copy()
    config_with_size["dataset_info"] = {
    ...
    with open(os.path.join(combined_results_dir, "hyperparameters.json"), "w", encoding="utf-8") as f:
        json.dump(config_with_size, f, indent=4, ensure_ascii=False)
    ...
    print(f"데이터셋 분할: 전체 {dataset_size}개 중")
    print(f"- 학습용: {train_size}개 (80%)")
    print(f"- 검증용: {val_size}개 (20%)")
