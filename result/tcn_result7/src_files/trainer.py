...
def save_source_code(dest_dir):
    ...
    # src 디렉토리의 Python 파일들 복사
    src_files_dir = os.path.join(dest_dir, "src_files")
    os.makedirs(src_files_dir, exist_ok=True)
    
    # src 디렉토리 내의 Python 파일들
    for file in glob.glob(os.path.join(src_dir, "src", "*.py")):
        shutil.copy2(file, src_files_dir)
    
    # train.py 파일 복사
    train_py = os.path.join(src_dir, "script", "train.py")
    if os.path.exists(train_py):
        shutil.copy2(train_py, src_files_dir)
    
    print(f"소스 코드가 {src_files_dir}에 저장되었습니다.")
