...
class TCN(nn.Module):
    def __init__(self, input_size, output_size, num_channels, kernel_size, dropout, use_se=False):
        ...
        self.no_activity_threshold = 0.95  # 0.97에서 0.95로 낮춤
        self.temporal_window = 11  # 9에서 11로 증가
        ...
        # standing과 no_activity 간의 전이 확률 조정
        self.transition_matrix[0, 3] = 0.6  # standing -> no_activity 더 제한 (0.8 -> 0.6)
        self.transition_matrix[3, 0] = 1.4  # no_activity -> standing 더 허용 (1.2 -> 1.4)
        # sitting과 no_activity 간의 전이 확률 조정
        self.transition_matrix[1, 3] = 0.6  # sitting -> no_activity 더 제한 (0.8 -> 0.6)
        self.transition_matrix[3, 1] = 1.4  # no_activity -> sitting 더 허용 (1.2 -> 1.4)
            ...
            if conservative_no_activity:
                ...
                for b in range(batch_size):
                    for t in range(seq_len):
                        ...
                        # 윈도우의 60%에서 55%로 완화 - 예측 안정성 개선
                        if predictions[b, t] == 3 and non_no_activity > (end-start) * 0.55:
                            ...
                            if t > 0 and predictions[b, t-1] != 3:
                                # 이전 프레임이 no_activity가 아니면 가중치 증가
                                probs_except_no_activity[predictions[b, t-1]] *= 1.25  # 1.2에서 증가
                            
                            if t < seq_len-1 and predictions[b, t+1] != 3:
                                # 다음 프레임이 no_activity가 아니면 가중치 증가
                                probs_except_no_activity[predictions[b, t+1]] *= 1.25  # 1.2에서 증가
