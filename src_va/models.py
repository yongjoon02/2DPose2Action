...
class TemporalBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, padding, dropout=0.2):
        ...
        # 가중치 정규화 제거하고 일반 Conv1d 사용
        self.conv1 = nn.Conv1d(n_inputs, n_outputs, kernel_size,
                               stride=stride, padding=padding, dilation=dilation)
        self.chomp1 = Chomp1d(padding)
        self.bn1 = nn.BatchNorm1d(n_outputs)  # 배치 정규화 추가
        self.relu1 = nn.ReLU()
        ...
        self.bn2 = nn.BatchNorm1d(n_outputs)  # 배치 정규화 추가
        self.relu2 = nn.ReLU()
...
class TCN(nn.Module):
    ...
    def forward(self, x):
        ...
        # 원래 형식으로 되돌림: [batch_size, seq_len, num_channels[-1]]
        y = y.transpose(1, 2)
        
        # 각 프레임에 대한 분류 수행
        output = self.linear(y)
        
        # 최종 출력: [batch_size, seq_len, num_classes]
        return output 